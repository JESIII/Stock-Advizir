...
def getRating(tickr):
    ...
    historicalPrice = web.get_data_yahoo(tickr, start=start, end=end)
    ...
    #print(end)
    # Get data
    data = web.DataReader(tickr, 'yahoo', start, end)
    #get macd signal
    stockDF = data.copy()
    ...
    keepPrice = historicalPrice["Close"]
    ...
    # print(type(y_pred))
    print(y_pred[-10:])
# lines = keepPrice.plot.line(y='Close')
# #Plot
# keepPrice['Rating'] = pd.DataFrame(y_pred)
# buyRating = keepPrice.query("Rating == 'Buy'")
# sellRating = keepPrice.query("Rating == 'Sell'")
